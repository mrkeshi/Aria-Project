                ...
                <div class="flex w-full flex-wrap max-md:justify-center justify-end mx-2 mt-6 ">
                    ...
                    <div class="flex flex-wrap ">
                        ...
                        <RouterLink :to="{name:'skil'}" class="flex items-center mx-2 px-4 py-2 font-medium tracking-wide text-white capitalize transition-colors duration-300 transform bg-green-600 rounded-lg hover:bg-blue-500 focus:outline-none focus:ring focus:ring-blue-300 focus:ring-opacity-80">
                            ...
                            <span class="mx-1 text-sm">مدیریت فیلد</span>
                        </RouterLink>
                        ...
                        <form class="flex flex-col " @submit.prevent="send">
                            ...
                            <div class="mt-4">
                                <div class="w-full  " v-if="skills[0]">   
                                  ...
                                  <h1 v-else style="color: red;" >تا زمانی که فیلدی تعریف نکنید امکان دعوت به پروژه را ندارید.</h1>

                            </div>

                            <div  v-if="skills[0]" class="w-full float-left">
                                <button v-show="!loading"  style="font-size: 14px !important;margin-top: 20px;" 
                                :class="[
...
const send = async () => {
    if (!loading.value) {
        loading.value=true
        Promise.resolve(userstore.add(auth.user.access, {
  email: user.email,
  skill: user.skil,
  level: user.level
}, userstore.user.current_project))
  .then(() => {
