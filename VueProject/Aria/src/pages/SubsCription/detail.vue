...
<div class="bg-white p-5 rounded-xl shadow border space-y-3">
  ...
  {{ 
    Sub.level === 3
      ...
      : `${toPersianNumber(Sub.countDetail.task || 0)} از ${toPersianNumber(totalTaskLimit)}`
  }}
</p>
  <div class="w-full h-2 bg-gray-200 rounded">
...
onMounted(async () => {
  ...
  if (Sub.level === 3) {
    ...
    const totalTaskLimitCalculated = projectCount * tasksPerProject.value;
  ...
  const totalUserLimitCalculated = projectCount * usersPerProject.value;
