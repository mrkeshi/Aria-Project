<template>
    <div>
        <div class="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8 px-4 ">
                <h1 class="text-gray-700 font-bold text-xl"> ویرایش کاربر </h1>
                        ...
                        <form class="flex flex-col " @submit.prevent="send">
                            <div class="mt-4">
                                ...
                                <div class="mt-3 md:flex md:items-center md:-mx-2">
                                    <button @click.prevent="changeRole(3)" :class="['flex justify-center  w-full px-6 py-3 text-blue-500 border border-blue-500  rounded-lg md:w-auto md:mx-2 focus:outline-none' ,  proj.role.level == 3 ? 'active' : '']">
                                    ...
                                    <button @click.prevent="changeRole(2)" :class="['flex justify-center  w-full px-6 py-3 text-blue-500 border border-blue-500  rounded-lg md:w-auto md:mx-2 focus:outline-none' ,  proj.role.level == 2 ? 'active' : '']">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            ...
                            <div class="mt-4">
                                ...
                                <input disabled  type="email" v-model="proj.role.user_email" class="block w-full px-5 py-3 mt-2 text-gray-700 placeholder-gray-400 bg-white border border-gray-200 rounded-lg dark:placeholder-gray-600 dark:bg-gray-900 dark:text-gray-300 dark:border-gray-700 focus:border-blue-400 dark:focus:border-blue-400 focus:ring-blue-400 focus:outline-none focus:ring focus:ring-opacity-40">
                            </div>
                            <div class="mt-4">
                                <div class="w-full ">   
...
onMounted(async()=>{
    await getSkils(User.user.access).then((data)=>{
      
        Object.assign(skills,{...data.data})
      
    })
    await proj.getRole(User.user.access,userStore.user.current_project,route.params.id)
})

function changeRole(level){
proj.role.level=level
}
const send = async () => {
    if (!loading.value) {
        loading.value=true
        await proj.editRole(User.user.access,userStore.user.current_project,route.params.id,proj.role )
        loading.value=false
