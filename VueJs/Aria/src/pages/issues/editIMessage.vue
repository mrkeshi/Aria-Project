<template>
    ...
    <span v-else="user.user.level==2"> کارمند {{ user.user.rollname }}  </span>
                        ...
                        <div class="max-lg:w-full  w-5/6 mx-auto bg-blue-100 p-4 rounded-md border border-blue-500 flex justify-center flex-col">
                            <b class="text-blue-600">قبل از ویرایش پاسخ دقت کنید</b>
                            <p class="text-blue-500 mt-3">
                              ۱- از ارسال پاسخ تکراری خودداری کنید.
                            </p>
...
۲- شما مجاز به آپلود فقط یک فایل هستید پس لطفا  در صورتی که بیش از چندفایل دارید لطفا آن ها را zip کرده و بارگذاری نمایید.
                        ...
                        <form @submit.prevent="send"   class="mt-4 max-lg:mt-1 p-8 w-full max-lg:p-6">
                          ...
                          <h2 class="p-4" style="font-size: 18px;" v-if="IssuesStore.singleMessage.message_parent">شما در حال پاسخ به 
                            <a :href="'#d'+IssuesStore.singleMessage.message_parent.id" class="text-blue-500">{{ IssuesStore.singleMessage.message_parent.respondent_name }}</a>
                            هستید
                          <span style="cursor: pointer;" @click="IssuesStore.singleMessage.message_parent=null" class="text-red-500">(حذف)</span>
                          </h2>
                            ...
                            <div class="flex justify-center p-2">
                              <textarea v-model="IssuesStore.singleMessage.description" rows="5" id="" class="w-full  mt-4 bg-[#f3f3f3] border  rounded-lg border-gray-300 focus:border-blue-500 p-4 text-gray-600 outline-blue-500" placeholder="چیزی برای نوشتن وارد کنید"></textarea>
                            ...
                            <div class="w-full mt-3 mb-6 px-2">
                             ...
                             <div v-if="IssuesStore.singleMessage.assigned_attached" >
                                <div style="align-items: end;" class="flex">

                               
                                <a :href="IssuesStore.singleMessage.assigned_attached" style="padding-left: 6px;" class="text-blue-500 px-8 flex items-center mt-4">
                          ...
                          <span class="mx-1">
                              ...
                              <button  :class="[
                                ...
                                loading ? ' disabled' : ''
                            ]" :disabled="loading">ویرایش پاسخ</button>
                            </div>
                        </form>
...
let av=null
  function handleFileUpload(event) {
    ...
    onMounted(async()=>{
     ...
     await IssuesStore.showMessage(route.params.id,user.user.current_project,route.params.messageid)
     if(IssuesStore.singleMessage.respondent!=user.user.id){
  ...
  message_parent:''
   ...
   async function send(){
    if (!loading.value) {
      message.description=IssuesStore.singleMessage.description
      message.message_parent=IssuesStore.singleMessage.message_parent
      console.log(message.message_parent)
      loading.value=true
      await IssuesStore.editMessage(route.params.id,user.user.current_project,route.params.messageid,message,av)
      loading.value=false
   
    }
  }
  function delfile(){
    IssuesStore.singleMessage.assigned_attached=''
    av=''
  }
