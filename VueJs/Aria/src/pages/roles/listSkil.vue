<template>
    <div>
        <div class="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8 px-4 ">
                <h1 class="text-gray-700 font-bold text-xl">صفحه مدیریت فیلد ها</h1>
                <p class="text-gray-600 text-sm text-wrap">شما در این قسمت می توانید حوزه های کاری پروژه جاری خودتان را مدیریت کنید..</p>
             
                <div class="flex w-full flex-wrap max-md:justify-center justify-between mx-2 mt-6 ">
                    <div>
                        <RouterLink :to="{name:'addskil'}" class="flex items-center px-4 py-2 font-medium tracking-wide text-white capitalize transition-colors duration-300 transform bg-yellow-500 max-md:my-2 rounded-lg hover:bg-blue-500 focus:outline-none focus:ring focus:ring-blue-300 focus:ring-opacity-80">
                            ...
                            <span class="mx-1 text-sm">تعریف حوزه جدید</span>
                        </RouterLink>
                    </div>
            ...
            <div class="overflow-hidden border border-gray-200 dark:border-gray-700 md:rounded-lg">
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200 dark:divide-gray-700 dark:bg-gray-900">
                        <tr v-for="st in skills">
                            <td class="px-4 py-4 text-sm font-medium whitespace-nowrap">
                                <div>
                                    <h2 class="font-medium text-gray-800 dark:text-white ">{{ st.name }}</h2>
                            ...
                            <td class="px-4 py-4 text-sm whitespace-nowrap">
                                <div>
                                    <h4 class="text-gray-700 dark:text-gray-200">{{ st.description }}</h4>
                            ...
                            <RouterLink :to="{name:'editskil',params:{id:st.id}}" class="text-sm text-white mx-2  p-2 bg-green-400 rounded-lg ">ویرایش</RouterLink>
                            <button @click="deleteId(st.id)" class="text-sm text-white  p-2 bg-red-500 rounded-lg ">حذف</button>
...
onMounted(async()=>{
    ...
    getSkils(user.user.access).then((data)=>{
